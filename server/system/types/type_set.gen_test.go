...
func TestOrderSetWalk(t *testing.T) {
	var (
		value = make(OrderSet, 3)
		req   = require.New(t)
	)

	// check walk with no errors
	{
		err := value.Walk(func(*Order) error {
			return nil
		})
		req.NoError(err)
	}
	...
	// check walk with error
	req.Error(value.Walk(func(*Order) error { return fmt.Errorf("walk error") }))
}

func TestOrderSetFilter(t *testing.T) {
	var (
		value = make(OrderSet, 3)
		req   = require.New(t)
	)
	...
	// filter nothing
	{
		set, err := value.Filter(func(*Order) (bool, error) {
			return true, nil
		})
		req.NoError(err)
		req.Equal(len(set), len(value))
	...
	// filter error
	{
		_, err := value.Filter(func(*Order) (bool, error) {
			return false, fmt.Errorf("filter error")
		})
		req.Error(err)
	}
...
func TestOrderSetIDs(t *testing.T) {
	var (
		value = make(OrderSet, 3)
		req   = require.New(t)
	)
